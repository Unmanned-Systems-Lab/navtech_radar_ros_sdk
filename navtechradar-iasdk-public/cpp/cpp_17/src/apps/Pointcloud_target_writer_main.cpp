...
int main(int argc, char* argv[])
{
    ...
    if (options.global_option("-h")) {
        stdout_log << options.usage() << endl;
        exit(0);
    }

    auto tcp_addr   = options.global_option("-i").translate_to<IP_address>();
    auto tcp_port   = options.global_option("-p").to_int<std::uint16_t>();
    auto filetag    = options.global_option("-f").value();
    auto rotations  = options.global_option("-r").to_int<std::uint16_t>();

    auto nav_threshold  = options.global_option("-t").to_float();
    auto window_size    = options.global_option("-w").to_int<std::uint16_t>();
    auto min_bin        = options.global_option("-b").to_int<std::uint16_t>();
    auto max_peaks      = options.global_option("-m").to_int<std::uint16_t>();

    Navtech::Navigation::Subresolution_mode sub_mode { options.global_option("-s").to_int<std::uint16_t>() };
