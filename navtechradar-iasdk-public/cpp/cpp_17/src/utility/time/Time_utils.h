...
namespace Navtech::Time {
    ...
    public:
        ...
        // Fractional value
        //
        constexpr std::uint64_t to_usec() const
        {
            return ticks();
        }
    ...
    constexpr Duration operator*(const Dur_Ty& lhs, double rhs)
    {
        ...
        // std::isnan() is only constexpr from C++ 23 onwards
        //
        if (rhs != rhs) throw std::invalid_argument { "RHS operand is not a number" };
    ...
    constexpr Duration operator*(double lhs, const Dur_Ty& rhs)
    {
        ...
        // std::isnan() is only constexpr from C++ 23 onwards
        //
        if (lhs != lhs) throw std::invalid_argument { "RHS operand is not a number" };
