...
namespace Navtech::Utility {
    ...
    public:
        ...
        Threadsafe_queue(std::size_t max_elems);

        template <typename U> bool push(U&& in_val);
        std::optional<T> pop();
    ...
    private:
        ...
        std::size_t             max_sz   { Threadsafe_queue::unlimited() };
    ...
    template <typename T>
    Threadsafe_queue<T>::Threadsafe_queue(std::size_t max_elems) :
        max_sz { max_elems }
    {
    }
    ...
    template <typename T>
    template <typename U> 
    bool Threadsafe_queue<T>::push(U&& in_val)
    {
        std::lock_guard lock { mtx };

        if (Queue::size() == max_sz) return false;
