...
namespace Navtech::Time {
    ...
    timespec Duration::to_ntp() const
    {
        auto ticks       = duration.count();
        auto seconds     = static_cast<std::time_t>(ticks / 1'000'000);
        auto sub_seconds = static_cast<long>((ticks - (seconds * 1'000'000)) * 1000);
    ...
    std::string Duration::to_string() const
    {
        ...
        // - Values greater than 1msec are displayed as (fractional) millisecond values
    ...
    namespace Monotonic {
        ...
        timespec Observation::to_ntp() const
        {
        ...
        void sleep_for(const Duration& sleep_period)
        {
            ...
            // Sleep (and therefore this_thread::sleep_for) on Windows is not particularly accurate.
            // This *SHOULD* be fixed in VS 2022 V17.9 and above, but this is included for 
            // backwards-compatibility with older versions of the compiler
            // For an explanation of the inaccuracy, see here: https://blog.bearcats.nl/accurate-sleep-function/
            // The following code snippet is based off of: https://blog.bearcats.nl/perfect-sleep-function/
            //
            #ifdef _WIN32
                auto t0 = std::chrono::high_resolution_clock::now();
                auto target = t0 + std::chrono::duration_cast<std::chrono::nanoseconds>(duration);
                // spin
    ...
    namespace Real_time {
        ...
        std::string Observation::to_string() const
        {
            ...
            //
            if (show_msec) {
                os << fixed << setw(3) << setfill('0');
                os << static_cast<unsigned int>(round(get_subseconds().to_usec() / 1000.0));
            }

            if (show_usec) {
                os << fixed << setw(6) << setfill('0');
                os << get_subseconds().to_usec();
            }
        ...
        timespec Observation::to_ntp() const
        {
            return timespec {
                to_time_t(),
                static_cast<long>(get_subseconds().to_usec() * 1000)
            };
        }
    ...
    namespace Thread_safe {
        ...
        Duration::Duration(Tick_type init) : Time::Duration { init }
        {
        }
        ...
        std::uint64_t Duration::to_usec() const
        {
            std::lock_guard lock { mtx };
        ...
        std::uint64_t Duration::microseconds() const
        {
            std::lock_guard lock { mtx };
        ...
        std::uint64_t Duration::milliseconds() const
        {
            std::lock_guard lock { mtx };
        ...
        std::uint64_t Duration::seconds() const
        {
            std::lock_guard lock { mtx };
