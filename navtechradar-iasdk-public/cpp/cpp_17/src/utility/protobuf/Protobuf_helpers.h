...
namespace Navtech::Protobuf {
	...
	std::optional<Protobuf_Ty> from_vector_into(const std::vector<uint8_t>& data, Format format = Format::binary)
	{
		...
		// De-serialise a vector of bytes back into a shared_owner protocol buffer.
	//
	template <class Protobuf_Ty>
	shared_owner<Protobuf_Ty> from_vector_into_shared(const std::vector<uint8_t>& data, Format format = Format::binary)
	{
		try {
			shared_owner<Protobuf_Ty> proto_buf { allocate_shared<Protobuf_Ty>() };

			std::string input_str { data.begin(), data.end() };

			if (format == Format::binary) {
				proto_buf->ParseFromString(input_str);
			}
			else {	
				google::protobuf::TextFormat::ParseFromString(input_str, proto_buf.get());
			}
