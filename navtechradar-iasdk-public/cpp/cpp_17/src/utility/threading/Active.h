...
namespace Navtech::Utility {
    ...
    protected:
        ...
        template <typename Return_Ty, typename Callable_Ty, typename... Arg_Ty>
        Return_Ty sync_call(Callable_Ty&& callable, Arg_Ty&&... arg)
        {
            std::packaged_task<Return_Ty()> fn { std::bind(callable, std::forward<Arg_Ty>(arg)...) };
            auto ret_val = fn.get_future();

            msg_queue.push(std::ref(fn));
