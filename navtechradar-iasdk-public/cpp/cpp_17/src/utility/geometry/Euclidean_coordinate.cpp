...
namespace Navtech::Euclidean {
    ...
    Spherical::Coordinate Coordinate::to_spherical() const
    {
        ...
        if (Utility::essentially_equal(y, 0.00f, 0.0001f)) {
            // Special case where x and y are both 0: The bearing is undefined.
            // This will never be the case with our radar, but must be considered for reusability
            // 
            // WARNING: THIS WILL CAUSE AN INFINITE LOOP IF COMPILING WITH --ffast_math
            //
            if (Utility::essentially_equal(x, 0.00f, 0.0001f)) bearing = numeric_limits<float>::quiet_NaN();
            else bearing = (x > 0.0f ? 1.0f : -1.0f) * pi<float> / 2.0f;
        }
        if (y < 0.0f) bearing = (x >= 0.0f ? 1.0f : -1.0f) * pi<float> + atan(x / y);
        if (y > 0.0f) bearing = std::atan(x / y);

        // if (x == 0.0f && y == 0.0f) bearing = std::numeric_limits<float>::quiet_NaN();
        // else bearing = (y >= 0.0f ? 1.0f : -1.0f) * acos(y / hypot(x, y));

        inclination = std::acos( z / range );
