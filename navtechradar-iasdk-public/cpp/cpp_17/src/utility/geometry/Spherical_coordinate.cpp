...
namespace Navtech::Spherical {
    ...
    Euclidean::Coordinate Coordinate::to_euclidean() const
    {
        return Euclidean::Coordinate {
            std::round((range * std::sin(bearing.to_radians()) * std::sin(inclination.to_radians())) * 1000.0f) / 1000.0f,
            std::round((range * std::cos(bearing.to_radians()) * std::sin(inclination.to_radians())) * 1000.0f) / 1000.0f,
            std::round((range * std::cos(inclination.to_radians())) * 1000.0f) / 1000.0f
        };
    }
    ...
    bool Coordinate::operator==(const Coordinate& rhs) const
    {
        ...
        (this->bearing == rhs.bearing) && (this->inclination == rhs.inclination);
    }
