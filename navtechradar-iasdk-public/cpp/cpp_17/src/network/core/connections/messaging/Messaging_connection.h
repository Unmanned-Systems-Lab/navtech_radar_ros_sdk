...
namespace Navtech::Networking::Messaging {
    ...
    void Receiver<protocol, transport, tls>::read_header()
    {
        ...
        try {
            ...
            if (Protocol_traits::is_valid(incoming_msg)) {
                ...
                stdout_log << Logging_level::debug;
                stdout_log << "Messaging receiver read_header() [" << id << "] invalid header" << endl;
            ...
            stdout_log << Logging_level::debug;
            stdout_log << "Messaging receiver [" << id << "] "
                       << "caught exception [" << e.code().value() << "][" << e.what() << "]" 
                       << endl;
    ...
    void Receiver<protocol, transport, tls>::dispatch()
    {
        ...
        try {
            ...
            stdout_log << Logging_level::debug;
            stdout_log << "Messaging receiver [" << id << "] "
                       << "caught exception [" << e.code().value() << "][" << e.what() << "]" << endl;
    ...
    {
        try {
            stdout_log << "Messaging connection, setting socket options..." << endl;

            Connection_traits::set_socket_options(socket);
        }
        catch (std::system_error& e) {
            stdout_log << "Messaging connection [" << ident << "] error setting socket options. "
                       << "Caught exception [" << e.code().value() << "][" << e.what() << "]" 
                       << endl;
        }
    }
