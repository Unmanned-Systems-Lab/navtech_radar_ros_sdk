...
namespace Navtech::Networking::CP_protocol {
    ...
    Message::Message(const Networking::IP_address& ip_addr, Message::ID id, const Message::Buffer& message) :
        address         { ip_addr },
        identity        { id }
    {
        replace(message);
    ...
    Message::Message(const Networking::IP_address& ip_addr, Message::ID id, Message::Buffer&& message) :
        address         { ip_addr },
        identity        { id }
    {
        replace(std::move(message));
    ...
    Message::Message(const Networking::IP_address& ip_addr, ID id, Const_iterator message_start, std::size_t message_sz) :
        address         { ip_addr },
        identity        { id }
    {
        replace(message_start, message_sz);
    ...
    {
        auto header = Header::overlay_onto(data.data());
        header->payload_size = htonl(static_cast<std::uint32_t>(data.size() - header_size()));
    }
    ...
    void Message::add_signature() const
    {
        using std::begin;
        using std::end;
        using std::copy;
        ...
        // Nasty hack to modify the data buffer, even though
        // this is a const object
        //
        auto self = const_cast<Message*>(this);

        copy(valid_signature().begin(), valid_signature().end(), self->signature_begin());
    }
