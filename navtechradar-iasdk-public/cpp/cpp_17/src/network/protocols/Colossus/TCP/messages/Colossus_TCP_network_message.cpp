...
namespace Navtech::Networking::Colossus_protocol::TCP {
    ...
    Message& Message::append(const Message::Buffer& protocol_buffer)
    {
        ...
        if (protocol_buffer.size() == 0) return *this;
    ...
    Message& Message::append(Const_iterator buffer_begin, Const_iterator buffer_end)
    {
        data.insert(data.end(), buffer_begin, buffer_end);
