...
class MessageToRadar(base_radar_message.BaseRadarMessage):
    ...
    message_to_radar_type_map =  {
                            ...
                            211 : "Change the Profile Registers"
                        }
    ...
    def __init__(self, message_type, *args):
        ...
        # Change Profile Registers
        elif self.message_type == 211:
            self.payload_size = 44
            self.payload = bytearray(base_radar_message.BaseRadarMessage.check_signature_bytes) + \
            int.to_bytes(self.version, 1, byteorder='big') + \
            int.to_bytes(self.message_type, 1, byteorder='big')  + \
            int.to_bytes(self.payload_size, 4, byteorder='big')  + \
            int.to_bytes(args[0], 4, byteorder='big') + \
            int.to_bytes(args[1], 4, byteorder='big') + \
            int.to_bytes(args[2], 4, byteorder='big') + \
            int.to_bytes(args[3], 4, byteorder='big') + \
            int.to_bytes(args[4], 4, byteorder='big') + \
            int.to_bytes(args[5], 4, byteorder='big') + \
            int.to_bytes(args[6], 4, byteorder='big') + \
            int.to_bytes(args[7], 4, byteorder='big') + \
            int.to_bytes(args[8], 4, byteorder='big') + \
            int.to_bytes(args[9], 4, byteorder='big') + \
            int.to_bytes(args[10], 4, byteorder='big')
