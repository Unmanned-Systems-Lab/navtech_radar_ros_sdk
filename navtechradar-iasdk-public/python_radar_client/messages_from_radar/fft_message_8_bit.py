#####################################################################################################################
# A python colossus message handling class. Used to handle an FFT message from the radar
#####################################################################################################################
...
class FftMessage8Bit(base_radar_message.BaseRadarMessage):
    ...
    def __init__(self, signature_data, header_data, payload_data):
        ...
        self.fft_data = np.asarray(self.payload_data[self.fft_data_offset:], dtype=np.uint8)
