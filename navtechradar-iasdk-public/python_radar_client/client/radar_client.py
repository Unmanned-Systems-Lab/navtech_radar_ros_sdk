#####################################################################################################################
# A python radar client. Used to manage the connection to a radar, and send/receive the most common radar message types
#####################################################################################################################
...
from message_handling import messages_from_radar
from message_handling import messages_to_radar
import copy
...
class RadarClient:
    ...
    # Constructor
    def __init__(self, ip, port, log_directory = "logs", log_filename = "radar_client_log"):
        self.ip = ip
        self.port = port
        ...
        self.incoming_radar_message_queue = Queue(maxsize = 100)
        self.outgoing_radar_message_queue = Queue(maxsize = 10)

        self.signature_data = []
        self.header_data = []
        self.payload_data = []

        self.log.write_to_log("Created radar client")
    ...
    def handle_messages(self):
        ...
        try:
            self.signature_data = list(self.client_socket.recv(messages_from_radar.MessageFromRadar.signature_length))
            if (self.signature_data != messages_from_radar.MessageFromRadar.check_signature):
                return
            self.header_data = list(self.client_socket.recv(messages_from_radar.MessageFromRadar.header_length))
            self.payload_size = int.from_bytes(self.header_data[2:6], byteorder='big')
            self.payload_data = list(self.client_socket.recv(self.payload_size))
            if (len(self.payload_data) == self.payload_size):
                self.incoming_radar_message_queue.put(messages_from_radar.MessageFromRadar(copy.deepcopy(self.signature_data), copy.deepcopy(self.header_data), copy.deepcopy(self.payload_data)))
        except Exception as e:
            self.log.write_to_log("Error reading message - {}".format(e))
            pass

        # Try to send an outgoing message, if any in queue
        if (self.outgoing_radar_message_queue.qsize() > 0):
            try:
                self.latest_outgoing_message = self.outgoing_radar_message_queue.get()
                self.log.write_to_log("Sending message type: {} ({}) to radar".format(self.latest_outgoing_message.message_type,
                self.latest_outgoing_message.message_type_alias))
                print("Sending message type: {} ({}) to radar\n".format(self.latest_outgoing_message.message_type,
                self.latest_outgoing_message.message_type_alias))
                self.client_socket.send(self.latest_outgoing_message.payload)
            except:
                pass
    ...
    def process_received_message(self):
        ...
        while True:
            ...
            # Read message type
            if (self.incoming_radar_message_queue.qsize() > 0):
                try:

                    self.latest_incoming_message = self.incoming_radar_message_queue.get()
    ...
    # Connect to the radar and start reading messages
    def connect(self):
    ...
    # Stop reading messages and disconnect from the radar
    def disconnect(self):
    ...
    def add_callback(self, message_type, callback_function, single_shot = False):
        messages_from_radar.MessageFromRadar.register_callback(message_type, callback_function, single_shot)
    ...
    def remove_callback(self, message_type):
        messages_from_radar.MessageFromRadar.remove_callback(message_type)
    ...
    def send_message(self, message_type, *args):
        self.outgoing_radar_message_queue.put(messages_to_radar.MessageToRadar(message_type, *args))