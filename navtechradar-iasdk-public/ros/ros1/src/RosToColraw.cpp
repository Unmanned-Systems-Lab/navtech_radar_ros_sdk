...
void WriteConfigToFile(float &range_res, uint16_t &range_in_bins, uint16_t &azimuths,uint16_t &encoder_size, uint16_t &rotation_speed,uint16_t &packet_rate,float &range_gain,float &range_offset)
{
    ...
    uint32_t Network_Top = Networking::to_uint32_host(Top);
    //uint32_t Network_Bottom = Networking::to_uint32_host(Bottom);
    uint32_t Network_Bottom = Bottom;    
    uint32_t Network_Payload_Size = Networking::to_uint32_host(Payload_Size);
    uint16_t Network_Azimuth_Samples = Networking::to_uint16_host(Azimuth_Samples);
    uint16_t Network_Bin_Size = Networking::to_uint16_host(Bin_Size);
    uint16_t Network_Range_In_Bins = Networking::to_uint16_host(Range_In_Bins);
    uint16_t Network_Encoder_Size = Networking::to_uint16_host(Encoder_Size);
    uint16_t Network_Rotation_Speed = Networking::to_uint16_host(Rotation_Speed);
    uint16_t Network_Packet_Rate = Networking::to_uint16_host(Packet_Rate);

    FloatToNetwork::v value;
    value.f=range_gain;
    uint32_t Network_Range_Gain = Networking::to_uint32_host(value.i);
    
    FloatToNetwork::v value2;
    value2.f=range_offset;
    uint32_t Network_Range_Offset = Networking::to_uint32_host(value2.i);
...
void DataHandler(const sensor_msgs::ImageConstPtr &radar_image_polar)
{
    ...
    if (firstpass)
        ...
        for(int az=0; az<azimuths;az++)
        {
            ...
            lowbits = Networking::to_uint32_host(lowbits);
            highbits = Networking::to_uint32_host(highbits);
            ...
            //Get Offset, Write Offset to buffer
                ...
                uint16_t Network_FFT_Data_Offset = Networking::to_uint16_host(FFT_Data_Offset);
            ...
            //Get Sweep Counter, increment per packet,rollover at max type, Write to buffer
                uint16_t Network_Sweep_Counter = Networking::to_uint16_host(Sweep_Counter);
            ...
            //Get Time, Write Seconds to buffer
                ...
                uint32_t Network_Seconds = Seconds;//Networking::to_uint32_host(Seconds);
            ...
            //Write Split Seconds to buffer
                ...
                uint32_t Network_Split_Seconds = SplitSeconds;//Networking::to_uint32_host(SplitSeconds);
