...
namespace CFAR {
    ...
    class Window {
    public:
        Window(std::size_t train_size, std::size_t guard_size, std::uint32_t threshold_delta);
    ...
    Window::Window(std::size_t train_size, std::size_t guard_size, std::uint32_t threshold_delta) :
        delta       { threshold_delta },
        train_sz  { train_size },
        guard_sz  { guard_size },
        window_sz   { train_size + guard_size + 1 }
    {
    }
    ...
    std::vector<std::uint8_t> cell_average(const std::vector<std::uint8_t>& azimuth, std::uint16_t num_train_cells, std::uint16_t num_guard_cells, std::uint16_t threshold_delta)
    {
        ...
        Window window { num_train_cells, num_guard_cells, threshold_delta };
...
Cfar_point_cloud_publisher::Cfar_point_cloud_publisher():Node{ "cfar_point_cloud_publisher" }
{
    ...
    declare_parameter("num_train_cells", 0);
    declare_parameter("num_guard_cells", 0);
    ...
    num_train_cells = get_parameter("num_train_cells").as_int();
    num_guard_cells = get_parameter("num_guard_cells").as_int();
...
void Cfar_point_cloud_publisher::fft_data_handler(Client& radar_client [[maybe_unused]], Message& msg)
{
    ...
    if ((azimuth_index >= start_azimuth) && (azimuth_index < end_azimuth)) {
        auto cfar_output = CFAR::cell_average(data, num_train_cells, num_guard_cells, threshold_delta);
...
void Cfar_point_cloud_publisher::configuration_data_handler(Client& radar_client [[maybe_unused]], Message& msg)
{
    ...
    RCLCPP_INFO(Node::get_logger(), "Num train cells: %i", num_train_cells);
    RCLCPP_INFO(Node::get_logger(), "Num guard cells: %i", num_guard_cells);
